...
class bird // Class for all the bird actions
{
  ...
  void drawBird() // Function to draw the bird
  {
    stroke(255, 255, 0); 
    noFill(); 
    strokeWeight(2); 
  ...
  void jump() // Function to make the bird jump or go up
  {
    ySpeed=-10;
  }
  void drag() // Function to make the bird fall or go down
  {
    ySpeed+=0.5;
  }
  void move() // Function to move the bird forward and pipes backward
  {
    yPos+=ySpeed; 
    for ( i=0; i<3; i++) 
    {
  ...
  void touch() // Function to check if the bird hits any pipe or not
  {
    if (yPos>800)
    {
      end=false;
...
class pipe // Class for all the pipe actions
{
  ...
  void drawPipe() // Function to draw the pipe
  {
    strokeWeight(5);
    stroke(0, 255, 0); 
  ...
  void currentPos() // Function to check the position of the bird with relavance to the pipe and to increase the score
  {
    if (xPos<0) {
      xPos+=(200*3); 
      gap = random(600)+100; 
...
void reset() // Function to reset the game and start again
{
  end=true; 
  score=0; 
  b.yPos=400; 
...
void mousePressed() // Function to start the game if mouse is clicked
{
  b.jump(); 
  start=false; 
  if (end==false)
...
void keyPressed() // Function to start the game if any key is pressed
{
  b.jump(); 
  start=false; 
  if (end==false)
...
void setup() // Function to initialize the game screen and the pipe object
{
  size(500, 800);
...
void draw() // Function to call the functions of classes bird and pipe and to run the game 
{
  background(0);
  k++;
  fill(0);
